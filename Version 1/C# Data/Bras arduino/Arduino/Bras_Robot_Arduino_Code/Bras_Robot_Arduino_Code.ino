...
bool right_position[4] = {false, false, false, false} ;

float angle_by_step [4] = {0.0133341156 , 0.0246405096, 0.14532872, 1};
//1/4.33^2
...
void send_to_usb(byte indice, int value)
{
  value = value <<4 + indice ;
...
void loop() 
{
  ...
  for(int m=0; m<4; m++)
  {
    ...
    {
      //Si on est bon on prévient
      if((consigne_moteur[m] - position_moteur[m])<angle_by_step[m] && (consigne_moteur[m] - position_moteur[m])>-angle_by_step[m])
      {
        if(!right_position[m])
        {
          right_position[m] = true ;  
          //Le channel 0 est reservé a dire quand on est arrivé           
          send_to_usb(0, m);        
        }
      }
      //Et sinon on corrige le tir 
      else {
        right_position[m] = false ; 
